...
const TeamSection = () => {
  ...
  const activeMembers = teamMembers.filter((member) => !member.hidden);
  const useGrid = activeMembers.length <= 3;
    ...
    <section className="py-12 md:py-20 px-4 md:px-6 bg-black relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className={`flex flex-col gap-10 mb-20 md:mb-24 ${
          useGrid ? 'text-center items-center' : 'md:flex-row md:items-end md:justify-between'
        }`}>
          <div className={useGrid ? 'text-center' : 'text-center md:text-left'}>
            <motion.h2 
        ...
        <div 
          ref={!useGrid ? scrollerRef : null} 
          className={
            useGrid 
              ? "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-5xl mx-auto justify-center"
              : "flex overflow-x-auto pb-8 gap-6 no-scrollbar hide-scrollbar scroll-smooth snap-x snap-mandatory"
          }
        >
          {activeMembers.map((member, i) => {
            ...
            return (
              <Link
                key={member.slug}
                to={`/team/${member.slug}`}
                className={useGrid ? "w-full block" : "flex-none w-[280px] md:w-[350px] snap-center block"}
              >
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  data-team-card
                  className="w-full liquid-glass rounded-3xl overflow-hidden group flex flex-col h-full border border-white/5 hover:border-red-500/50 hover:bg-black/60 hover:backdrop-blur-2xl transition-all duration-700 cursor-pointer shadow-xl"
                >
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="p-6 mt-auto flex items-end justify-between">
                      ...
                      <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/40 group-hover:text-white group-hover:border-red-500/50 group-hover:bg-red-500/10 transition-all duration-500">
